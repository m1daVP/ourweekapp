...
describe("error handler Sentry reporting", () => {
  ...
  it("captures unexpected errors with a route pattern, never the raw request URL", async () => {
    ...
    expect(context).toEqual({
      tags: { "http.method": "GET", "http.route": "/unexpected" },
    });
  ...
  it("preserves safe details in 4xx ApiError responses", async () => {
    ...
    expect(response.json()).toEqual({
      message:
        "Зустріч змінилася під час збереження підсумку. Оновіть дані та спробуйте ще раз.",

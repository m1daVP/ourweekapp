...
| Google Calendar sync                                                   | Calendar Sync settings route                                | `/v1/calendar/google/*`                                                                                                   | Calendar              | Included in public v1 | Keep production exposure behind `VITE_ENABLE_GOOGLE_CALENDAR=true` until step 7 verification |
...
- Development-only test copy may remain only behind
...
- Google Calendar sync must be omitted or hidden from production builds unless
  `VITE_ENABLE_GOOGLE_CALENDAR=true` is set for a verified backend build.

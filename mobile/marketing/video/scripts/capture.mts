...
const loadTargets = (): CaptureTarget[] => {
  ...
  if (!Array.isArray(value) || value.some((target) => !target || typeof target !== 'object' || typeof (target as CaptureTarget).file !== 'string' || typeof (target as CaptureTarget).url !== 'string')) {
    throw new Error('VIDEO_CAPTURE_TARGETS_JSON must be a JSON array of {file, url} objects');
  }
...
const main = async () => {
  ...
  try {
    ...
    const loginContext = await browser.newContext({viewport: {width: 432, height: 768}, deviceScaleFactor: 2});
    const loginPage = await loginContext.newPage();
    await loginPage.goto(new URL('/sign-in', baseUrl).toString(), {waitUntil: 'domcontentloaded'});
    await loginPage.locator('input[type="email"]').waitFor({state: 'visible', timeout: 10_000});
    ...
    try {
      await loginPage.waitForURL((url) => !url.pathname.endsWith('/sign-in'), {timeout: 10_000, waitUntil: 'commit'});
    } catch {
      throw new Error(`Demo sign-in did not complete: ${(await loginPage.locator('body').innerText()).replace(/\s+/g, ' ').slice(0, 500)}`);
    }
    ...
    for (const target of targets) {
      ...
      await page.goto(new URL(target.url, baseUrl).toString(), {waitUntil: 'domcontentloaded'});
      await page.waitForTimeout(1000);
      const recording = page.video();
      await page.waitForTimeout(2000);
      await context.close();
      if (!recording) throw new Error(`No recording was created for ${target.file}`);

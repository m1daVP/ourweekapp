...
export const toSrt = (cues: CaptionCue[], fps: number): string => {
  const ordered = [...cues].sort((left, right) => left.startFrame - right.startFrame);
  for (let index = 1; index < ordered.length; index += 1) {
    if (ordered[index - 1].endFrame > ordered[index].startFrame) throw new Error('Caption cues must not overlap');
  }

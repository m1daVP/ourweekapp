...
describe('promo content boundaries', () => {
  it('converts seconds using the exact composition frame rate', () => expect(secondsToFrames(1.5, 30)).toBe(45));
  it('keeps the flagship at or below 1:55', () => expect(getBriefDurationFrames(shipatonBrief)).toBeLessThanOrEqual(115 * 30));
  ...
  it('uses only approved Shipaton categories and no OneSignal claim', () => {
    expect(shipatonBrief.categories).toEqual(['Grand Prize', 'RevenueCat Design Award', 'HAMM Award', 'RevenueCat Peace Prize', '#BuildInPublic Award']);

...
for (const entry of groupEntries
  ...
  if (!identifierPattern.test(entry.name))
    throw new Error(
      `Avatar group "${entry.name}" must use lowercase kebab-case.`
    );
  ...
  if (files.length === 0)
    throw new Error(
      `Avatar group "${entry.name}" must contain at least 1 WebP file.`
    );
  const avatars = files.map((file) => {
    const id = file.name.slice(0, -'.webp'.length);
    if (!identifierPattern.test(id))
      throw new Error(
        `Avatar filename "${file.name}" must use lowercase kebab-case.`
      );
    if (avatarIds.has(id))
      throw new Error(`Avatar ID "${id}" must be unique across all groups.`);
    avatarIds.add(id);
    return {
      id,
      path: `/src/${relative(join(process.cwd(), 'src'), join(groupPath, file.name)).split(sep).join('/')}`,
    };
  });
...
if (groups.length === 0)
  throw new Error('Avatar catalog must contain at least 1 group.');

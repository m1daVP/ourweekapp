...
for (const entry of groupEntries.filter((item) => item.isDirectory()).sort((a, b) => a.name.localeCompare(b.name))) {
  ...
  if (files.length !== 4) throw new Error(`Avatar group "${entry.name}" must contain exactly 4 WebP files.`);
...
if (groups.length !== 5) throw new Error(`Avatar catalog must contain exactly 5 groups; found ${groups.length}.`);

  ...
  picker = iro.ColorPicker(pickerElement.value, {
    ...
    layout: [
      { component: iro.ui.Wheel },
      {
        ...
        options: { sliderSize: 24, sliderType: 'value' },
      },
    ],
  });
...
<template>
  ...
  >
    <section class="avatar-picker">
      <section>
        ...
        <div class="avatar-picker__colors">
          <button
            ...
            type="button"
            :aria-label="
              t('settings.selectedAvatarColor', { color: color.toUpperCase() })
            "
          ...
          >
            <span
              class="avatar-picker__custom-color-swatch"
              aria-hidden="true"
            />
          </button>
        </div>
          ...
          <div v-if="showCustomColor" class="avatar-picker__custom-panel">
            ...
            <label>
              ...
              <input
                v-model="draftColor"
                maxlength="7"
                @input="updateCustomColorDraft"
              />
            </label>
            <p v-if="hexError" class="meeting-error" role="alert">
              {{ hexError }}
            </p>
          </div>
        </Transition>
        ...
        >
          ...
          <div class="avatar-picker__grid">
            <button
              ...
              :aria-pressed="
                draftMode === 'avatar' && draftAvatarType === avatar.id
              "
      ...
      <button
        class="meeting-primary avatar-picker__confirm"
        type="button"
...
.avatar-picker button[aria-pressed='true'] {
  border-color: var(--color-primary);
}
.avatar-picker__colors span {
  width: 32px;
  height: 32px;
  border-radius: 50%;
}
...
.avatar-picker__group + .avatar-picker__group {
  margin-top: var(--space-3);
}
.avatar-picker__custom-panel {
  display: grid;
  gap: var(--space-2);
  margin-top: var(--space-3);
}
.avatar-picker__wheel {
  min-height: 280px;
  overflow: hidden;
...
.avatar-picker__custom-panel label {
  display: grid;
  gap: var(--space-1);
  font-weight: 700;
}
.avatar-picker__custom-panel input {
  min-height: 44px;
...
.custom-color-enter-active,
.custom-color-leave-active {
  overflow: hidden;
  transition:
...
.custom-color-enter-from,
.custom-color-leave-to {
  max-height: 0;
  opacity: 0;
  transform: translateY(-8px);
}
.custom-color-enter-to,
.custom-color-leave-from {
  max-height: 420px;
  opacity: 1;

...
import type { Meeting, MeetingTask } from '@/features/meeting/types'
...
const isLocked = computed(() => Boolean(meeting.value && !canAccessMeeting.value))
...
function getTaskResponsibleLabel(task: MeetingTask) {
...
<template>
  <section class="page-stack meeting-details-page">
    ...
    <template v-else>
      <header class="meeting-details-header">
        <div>
          ...
          <p class="page-copy">
            ...
            {{ allTasks.length }} tasks - {{ allAgreements.length }} agreements
          </p>
        </div>
        <button
      ...
      <PremiumLock feature="aiSummary">
        <section class="meeting-panel">
          <h2>AI summary</h2>
          <p class="meeting-empty">
            AI meeting summaries are planned for premium users and are not
            implemented yet.
          </p>
        </section>
      </PremiumLock>
      ...
      >
        ...
        <div class="meeting-summary__group">
          <h3>Tasks</h3>
          <ul v-if="section.tasks.length" class="meeting-list meeting-task-list">
